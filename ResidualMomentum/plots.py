...
def _x(g: pd.DataFrame):
    ...
    return pd.to_datetime(g["yy"].astype(int).astype(str) + "-" + g["mm"].astype(int).astype(str) + "-01") + pd.offsets.MonthEnd(0)
...
def plot_gross_vs_net(backtest: pd.DataFrame, out_dir: Path):
    ...
    for method, g in d.groupby("method", sort=False):
        ...
        plt.title(f"Plot C: Gross vs Net Wealth - {method}")
        plt.ylabel("Wealth")
        plt.legend()
        plt.tight_layout()
        plt.savefig(out_dir / f"plot_C_gross_vs_net_{safe_method_name(method)}.png", dpi=200)
        plt.close()
...
def plot_turnover(backtest: pd.DataFrame, out_dir: Path, rolling: int = 12):
    ...
    for method, g in backtest.groupby("method", sort=False):
        ...
        plt.plot(_x(h), h["turnover"].rolling(rolling, min_periods=1).mean(), label=method)
    plt.title(f"Plot E: Turnover, {rolling}-Month Rolling Average")
    plt.ylabel("Turnover")
    plt.legend()
    plt.tight_layout()
...
def plot_summary_metrics(backtest: pd.DataFrame, out_dir: Path):
    ...
    cols = ["sharpe_gross_ann", "sharpe_net_ann", "avg_turnover", "max_drawdown_net"]
