...
# Columns written to disk in streaming mode. Everything else is reconstructable
# from the yy/mm filename or irrelevant for the optimizer's stock-level turnover.
_STREAM_KEEP_COLS = ("permno", "node_id", "tilt_stock_w", "base_stock_w")


def _flush_month_stock_weights(
    weight_rows: list[pd.DataFrame],
    stock_weights_dir: Path,
    yy: int,
    mm: int,
) -> None:
    """Write one month's stock weights to disk, stripped to minimal columns."""
    if not weight_rows:
        return
    df = pd.concat(weight_rows, ignore_index=True)
    keep = [c for c in _STREAM_KEEP_COLS if c in df.columns]
    df = df[keep]
...
def _node_return_and_weights(
    ...
    stats = {
        ...
        "n_stocks": int(len(d)),
        "within_node_tilt_l1": float(
            np.abs(d["tilt_stock_w"] - d["base_stock_w"]).sum()
        ),
    }
...
def build_all_ap_tree_candidate_returns(
    ...
    stock_weights_dir: str | Path | None = None,
    ...
    if stock_weights_dir is not None:
        stream_dir = Path(stock_weights_dir)
    ...
    for month_idx, ((yy, mm), df_m0) in enumerate(grouped, start=1):
        ...
        if stream_dir is not None:
            _flush_month_stock_weights(weight_rows, stream_dir, int(yy), int(mm))
            # Free month-level membership rows immediately to keep RAM bounded.
            weight_rows.clear()
    ...
    if stream_dir is not None:
        ...
        stock_weights = pd.concat(weight_rows, ignore_index=True)
    else:
        stock_weights = pd.DataFrame()
