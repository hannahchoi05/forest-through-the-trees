...
def load_market_proxy(factor_dir: Path) -> pd.Series | None:
    ...
    Kept for compatibility with the teammate's pipeline. Pure AP-Trees does not
    need a market proxy, but this module is shared.
    """
...
def load_yahoo_monthly_benchmark(
    ...
    Used by the combined cross-method comparison; not called directly in the
    pure AP-Trees pipeline (benchmark is added once on the combined side).
    """
    if yf is None:
        raise ImportError("yfinance is not installed; cannot load Yahoo benchmark.")
