...
def _load_rf(factor_dir) -> np.ndarray | None:
    """
...
def main() -> None:
    ...
    # Load risk-free rate once here and pass it through to the optimizer.
    # This matches R Step3_RmRf_Combine_Trees.R which subtracts rf before
    # AP-pruning. rf is None if rf_factor.csv is missing (fallback to raw returns).
    rf_series = _load_rf(FACTOR_DIR)
